...
def _mgr(tmp_path: Path, peo_id: str = "624*D", env: str = "uat") -> PermissionManager:
...
def test_consent_file_scoped_to_peo_and_environment(tmp_path: Path) -> None:
    mgr_uat = _mgr(tmp_path, peo_id="624*D", env="uat")
    mgr_uat.grant([Scope.CLIENT_READ])

    mgr_prod = _mgr(tmp_path, peo_id="624*D", env="prod")
